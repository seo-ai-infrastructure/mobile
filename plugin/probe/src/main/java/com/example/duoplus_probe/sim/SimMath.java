...
public final class SimMath {
    ...
    public static double stepPhase(Scenario scenario,double timeMs) {
        double total=0;List<Scenario.ActivityEvent> events=scenario.activities;
        for(int i=0;i<events.size();i++){
            Scenario.ActivityEvent event=events.get(i);double end=i+1<events.size()?events.get(i+1).timeMs:scenario.durationMs;
            if(event.timeMs>=timeMs)break;
            total+=Math.max(0,Math.min(timeMs,end)-event.timeMs)/1000*event.cadenceHz;
        }
        return total;
    }
}

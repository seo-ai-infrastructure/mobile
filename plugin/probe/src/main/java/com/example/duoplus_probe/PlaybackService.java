...
public final class PlaybackService extends Service {
    ...
    private volatile PlaybackEngine engine;
    ...
    public final class LocalBinder extends Binder {
        ...
        public PlaybackEngine.Preview preview(){
    ...
    @Override public void onCreate(){
        super.onCreate();thread=new HandlerThread("scenario-deadlines");thread.start();worker=new Handler(thread.getLooper());
    ...
    private Notification notification(String text,boolean paused){
        ...
        return new Notification.Builder(this,CHANNEL).setSmallIcon(android.R.drawable.ic_media_play).setContentTitle("DuoPlus · local simulation").setContentText(text).setContentIntent(open)
        ...
        importTask=io.submit(()->{
            try{
                ...
                worker.post(()->{
                    if(generation!=importGeneration.get()||destroyed)return;
                    try{
                        worker.removeCallbacks(tick);releaseWake();engine=new PlaybackEngine(prepared);

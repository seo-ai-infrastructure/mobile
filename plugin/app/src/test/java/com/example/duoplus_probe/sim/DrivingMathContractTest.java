...
public final class DrivingMathContractTest {
    ...
    @Test public void imuSigmaIsZeroRegardlessOfRadioNoiseOptionOrSkippedFrames() throws Exception {
        ...
        for (String field : new String[] {"gravity_mps2", "linear_accel_mps2", "accelerometer_mps2", "gyro_rads"}) {
            assertEquals(field + " must have sigma zero in v1", noiseless.get(field), expected.get(field));
            assertEquals(field + " must not depend on delivery count", expected.get(field), actual.get(field));
        }
    }
}

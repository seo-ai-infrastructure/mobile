...
`MapProjectionTest` covers Mercator limits, perspective inverse/heading, offset viewports, dateline wrapping, visible-tile intersection, speed zoom, and tile-selection bounds. `RouteOverlayTest` covers clipping, exact source-vertex/dwell retention, short-arc dateline subdivision, and cancelled preparation. These JVM tests do not contact tile servers. Full visual/lifecycle verification belongs to the phone and car integration checks, including offline fallback and visible attribution.
